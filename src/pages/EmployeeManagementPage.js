...
const EmployeePage = () => {
    ...
    const fetchData = useCallback(async (filterPhongBan = '', currentSearchTerm = '') => {
        ...
        try {
            ...
            const [empRes, pbRes, cvRes, cnRes, tdRes, hdRes] = await Promise.all([
                api.get(employeeUrl), api.get('/PhongBan'), api.get('/ChucVuNhanVien'),
                api.get('/ChuyenNganh'), api.get('/TrinhDoHocVan'), api.get('/HopDong')
            ]);
            
            setEmployees(empRes.data); setPhongBans(pbRes.data); setChucVus(cvRes.data);
            setChuyenNganhs(cnRes.data); setTrinhDoHocVans(tdRes.data); setHopDongs(hdRes.data);
        } catch (error) { console.error("Failed to fetch data", error); }
        finally { setLoading(false); }
    }, []);

    useEffect(() => {
        const timer = setTimeout(() => fetchData(selectedPhongBan, searchTerm), 500);
        return () => clearTimeout(timer);
    }, [fetchData, selectedPhongBan, searchTerm]);
    
    const handleFilterChange = (e) => setSelectedPhongBan(e.target.value);
    const handleSearchChange = (e) => setSearchTerm(e.target.value);
    const handleSearchSubmit = (e) => { e.preventDefault(); fetchData(selectedPhongBan, searchTerm); };
    ...
    const handleAdd = () => { setCurrentEmployee(null); setIsEditModalOpen(true); };
    ...
    const handleAction = async (actionType, employee) => {
        ...
        if (actionType === 'edit' || actionType === 'view') {
          ...
          } catch (error) { alert("Không thể tải chi tiết nhân viên."); }
        }
    };
    ...
    const handleSave = async (employeeData, imageFile) => {
        ...
        try {
            if (imageFile) {
                ...
                const uploadRes = await api.post('/NhanVien/UploadImage', formData, { headers: { 'Content-Type': 'multipart/form-data' }});
    ...
    return (
        <DashboardLayout>
                    ...
                    <div className="header-right-panel">
                        ...
                        <div className="filter-container">
                            <select id="phongban-filter" value={selectedPhongBan} onChange={handleFilterChange}>
                                ...
                                {phongBans.map(pb => (<option key={pb.maPhongBan} value={pb.maPhongBan}>{pb.tenPhongBan}</option>))}
                            </select>
                        </div>
                ...
                {loading ? <p>Đang tải...</p> : (
                    <div className="table-container">
                        <table className="employee-table">
                            <thead><tr><th>Ảnh</th><th>Họ tên</th><th>Mã NV</th><th>Chức vụ</th><th>Phòng ban</th><th>Trạng thái</th></tr></thead>
                            <tbody>
                                {employees.map(emp => (
                                    <tr key={emp.maNhanVien}>
                                        <td>{emp.hinhAnh ? (<img src={getImageUrl(emp.hinhAnh)} alt={emp.hoTen} className="table-avatar" />) : (<FaUserCircle size={30} style={{ color: 'var(--toggle-icon-color)' }} />)}</td>
                                        <td onContextMenu={(e) => handleContextMenu(e, emp)} className="employee-name-cell">{emp.hoTen}</td>
            ...
            {contextMenu.visible && <ContextMenu x={contextMenu.x} y={contextMenu.y} onShowInfo={() => handleAction('view', currentEmployee)} onEdit={() => handleAction('edit', currentEmployee)} onDelete={() => handleAction('delete', currentEmployee)} onClose={closeContextMenu} />}
            ...
            {(isViewModalOpen || isEditModalOpen) && (
                <EmployeeModal 
                    ...
                    phongBans={phongBans} chucVus={chucVus} chuyenNganhs={chuyenNganhs}
                    trinhDoHocVans={trinhDoHocVans} hopDongs={hopDongs}
                />
            )}
        </DashboardLayout>
    );
