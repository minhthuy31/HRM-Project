                ...
                <form onSubmit={handleSubmit}>
                    <div className="avatar-uploader">
                        ...
                        <div className="avatar-preview" onClick={() => !isViewOnly && fileInputRef.current.click()}>
                            {previewUrl ? (<img src={getImageUrl(previewUrl)} alt="Avatar" />) : (<span><FaUserCircle size={60} /><br />Chọn ảnh</span>)}
                        </div>
                    </div>
                    <div className="employee-form">
                        ...
                        <div className="form-group"><label>Quê quán</label><input type="text" name="queQuan" value={formData.queQuan || ''} onChange={handleChange} disabled={isViewOnly} /></div>
                        <div className="form-group"><label>Địa chỉ thường trú</label><input type="text" name="diaChiThuongTru" value={formData.diaChiThuongTru || ''} onChange={handleChange} disabled={isViewOnly} /></div>
                        <div className="form-group"><label>Địa chỉ tạm trú</label><input type="text" name="diaChiTamTru" value={formData.diaChiTamTru || ''} onChange={handleChange} disabled={isViewOnly} /></div>
                        ...
                        <div className="form-group"><label>Số tài khoản NH</label><input type="text" name="soTaiKhoanNH" value={formData.soTaiKhoanNH || ''} onChange={handleChange} disabled={isViewOnly} /></div>
                        <div className="form-group"><label>Tên ngân hàng</label><input type="text" name="tenNganHang" value={formData.tenNganHang || ''} onChange={handleChange} disabled={isViewOnly} /></div>

                        <h3 className="form-section-title">Thông tin công việc</h3>
                        <div className="form-group"><label>Phòng ban</label><select name="maPhongBan" value={formData.maPhongBan || ''} onChange={handleChange} disabled={isViewOnly}><option value="">-- Chọn --</option>{(phongBans || []).map(pb => <option key={pb.maPhongBan} value={pb.maPhongBan}>{pb.tenPhongBan}</option>)}</select></div>
                        ...
                        {!isViewOnly && (<div className="form-group full-width"><button type="submit" className="btn-submit">{employee ? 'Lưu thay đổi' : 'Thêm mới'}</button></div>)}
                    </div>
                </form>
            </div>
        </div>
